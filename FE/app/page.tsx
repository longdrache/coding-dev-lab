  ...
  async function submitCode(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !result.token)
        throw new Error(result.message ?? "Không thể gửi code đến server.");
    ...
    <main className="min-h-screen bg-[#f5f1e8] px-5 py-8 text-[#17211b] sm:px-10">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 flex items-end justify-between border-b border-[#17211b]/20 pb-5">
          ...
          <span className="hidden text-sm text-[#17211b]/60 sm:block">
            Judge0 playground
          </span>
        </header>
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
            <section className="overflow-hidden rounded-sm border border-[#17211b]/15 bg-[#202a24] shadow-[8px_8px_0_#d65a3a]">
              ...
              <textarea
                ...
                className="min-h-[390px] w-full resize-y bg-transparent p-5 font-mono text-sm leading-7 text-[#f3efe3] outline-none placeholder:text-white/30"
              />
            </section>
